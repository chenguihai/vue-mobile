...
import { PopupMixin } from 'vant/es/mixins/popup'
...
import {eventPath} from '@/utils/tool'
...
export default createComponent({
	...
	methods: {
		...
		onWrapperTouchEnd: function onWrapperTouchEnd(event) {
			...
			const _video = document.querySelector('.van-image-preview__cover--_van_video')
			const hasVideo = path.includes(_video)
			...
			if (deltaTime < 300 && offsetX < 10 && offsetY < 10 && !hasVideo) {
	...
	render: function render() {
		...
		let _gens = [this.genImages(), this.genIndex()]
		if (!this.images.includes('_video')) _gens.push(this.genCover())
		return h(
			'transition',
			{
				attrs: {
