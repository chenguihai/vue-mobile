<template>
  <div class="m-category-chioced">
    <van-nav-bar :title="type==1?'查看类目':'投放区域'" left-arrow @click-left="onClickLeft" />
    <div v-if="type==1">
      <h3 class="m-title">
      ...
      <div :class="moreShow ? 'm-choiced-ul-wrap' : 'm-choiced-hidde'">
        <div class="m-ul-choiced" v-if="list.length>0">
          <ul>
            <li v-for="(itemSon,idx) in list" :key="idx">{{itemSon.cate_name}}</li>
          </ul>
        </div>
    ...
    <div v-else>
      <div :class="moreShow ? 'm-choiced-ul-wrap' : 'm-choiced-hidde'">
        <div class="m-ul-choiced" v-if="showList.length>0">
          <ul>
...
export default {
  ...
  mounted() {
    let num = 0;
    let { isDetail, type } = this.$route.query;
    this.isDetail = isDetail;
    this.type = type;
    if (type == 1) {
      this.showList = sessionStorage.getItem("selectShowList")
        ? JSON.parse(sessionStorage.getItem("selectShowList"))
        : [];
      this.showList.child.forEach(res => {
        let showListChild = res.child.filter(item => item.selected == true);
        this.list = showListChild;
        if (showListChild.length > 13) {
          this.moreShow = false;
        } else {
          this.moreShow = true;
        }
