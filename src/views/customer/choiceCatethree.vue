<template>
  <div class="container">
    ...
    <fixed-btn name="保存" @submit="handleSubmit"></fixed-btn>
  </div>
</template>
...
<script>
    ...
    export default {
        ...
        created() {
            ...
            if (categoryExample) {
                let category = JSON.parse(categoryExample);
                if (category.length === 0) {
                    this.$router.push({path: '/customer/selectCate'})
                } else {
                    const [one, two = 0, three = 0] = id.split(',');
                    this.selectTwoId = +two || 0;
                    this.selectThreeId = +three || 0;
                    let data = category[index];//分类
                    this.selectId = data.id;
                    this.oneCate = data;
                    this.twoCate = data.children;
                    for (let i = 0; i < data.children.length; i++) {
                        if (data.children[i].id === this.selectTwoId) {
                            this.selectTwo = i;
                            break;
                        }
                    }
                    this.threeCate = data.children[this.selectTwo].children;
                }
            }
        ...
        methods: {
            handleSubmit() {
                ...
                // this.$router.push({path: "/publishExample", query: {cate_id: `${this.selectId},${two},${three}`}});
                ...
                this.$router.push({path: "/publishExample", query: {cate_id: three.id}});
            },
            clickTwoCate(index, id) {
                ...
                this.selectTwoId = id;
                this.selectThree = 0;
                this.threeCate = this.twoCate[index].children;
            },
